...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Newest parseable `mcp` command node in the legacy conversation node list. */
    ...
    function McpCard(props) {
      ...
      // `useSession` is a selector hook: pass an explicit selector that yields
      // the legacy conversation node list (the card re-renders on its change).
      const nodes = typeof useSession === 'function'
        ? useSession((snapshot) => snapshot.chat.legacy.nodes)
        : null
      ...
      const toggle = async (server) => {
        ...
        try {
          ...
          // commands.execute contract: (agent, line, images, signal?) — images
          // is a JSON business argument introduced by newer DSH releases.
